...
protocol TFUserInfoHeaderVCDelegate: class {
    func updateUserInfo(for user: User)
    func setHeaderHeight(with height: CGFloat)
}

class TFUserInfoHeaderVC: UIViewController {

    // UI Elements
    let avatarImageView             = TFAvatarImageView(frame: .zero)
    let channelStatusImageView      = TFChannelStatusImageView(frame: .zero)
    let usernameLabel               = TFTitleLabel(textAlignment: .left, fontSize: DeviceTypes.isiPhoneSE || DeviceTypes.isiPhone8Zoomed ? 20 : 26, frame: .zero)
    let broadcasterTypeImageView    = UIImageView()
    let broadcasterTypeLabel        = TFSecondaryTitleLabel(textAlignment: .left, fontSize: 18, frame: .zero)
    let descriptionLabel            = TFBodyLabel(textAlignment: .left, frame: CGRect(x: 0, y: 0, width: UIScreen.main.bounds.size.width - 20, height: CGFloat.greatestFiniteMagnitude))
    ...
    weak var delegate: TFUserInfoHeaderVCDelegate!
    
    
    init(user: User, channel: Channel?, delegate: TFUserInfoHeaderVCDelegate) {
