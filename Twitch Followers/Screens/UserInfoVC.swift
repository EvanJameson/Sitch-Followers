...
class UserInfoVC: UIViewController {
    ...
    func configureUIElements(user: User, channel: Channel?) {
        self.add(childVC: TFChannelItemVC(user: user,channel: channel, delegate: self), to: self.itemViewOne)
        self.add(childVC: TFFollowerItemVC(user: user, delegate: self), to: self.itemViewTwo)
        self.add(childVC: TFUserInfoHeaderVC(user: user, channel: channel, delegate: self), to: self.headerView)
    }
    ...
    @objc func addButtonTapped() {
        guard let favorite = user else {
            self.presentTFAlertOnMainThread(title: "Something went wrong", message: TFError.unableToFavorite.rawValue, buttonTitle: "Ok")
            return
        }
...
extension UserInfoVC: TFChannelItemVCDelegate {    
...
extension UserInfoVC: TFFollowerItemVCDelegate {
...
extension UserInfoVC: TFUserInfoHeaderVCDelegate {
