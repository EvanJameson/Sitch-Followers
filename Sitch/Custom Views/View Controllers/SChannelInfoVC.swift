...
class SChannelInfoVC: UIViewController {

    // UI Elements
    let gameImageView       = SGameImageView(frame: .zero)
    let gameNameLabel       = STitleLabel(textAlignment: .left, fontSize: 24, frame: CGRect(x: 0, y: 0, width: 250, height: CGFloat.greatestFiniteMagnitude))
    let streamTitleLabel    = SSecondaryTitleLabel(textAlignment: .left, fontSize: 18, frame: CGRect(x: 0, y: 0, width: 250, height: CGFloat.greatestFiniteMagnitude))
