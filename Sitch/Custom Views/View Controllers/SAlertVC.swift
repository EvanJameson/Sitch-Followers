...
class SAlertVC: UIViewController {

    // UI Elements
    var containerView       = SAlertContainerView()
    let titleLabel          = STitleLabel(textAlignment: .center, fontSize: 20, frame: .zero)
    let messageLabel        = SBodyLabel(textAlignment: .center, frame: .zero)
    let actionButton        = SButton(backgroundColor: .systemPink, title: "Ok")
